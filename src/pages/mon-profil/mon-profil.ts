...
export class MonProfilPage {
  ...
  constructor(
    ...
    this.utilisateurSubscription =
      this.authService.utilisateurSubject.subscribe(
        (utilisateur: Utilisateur) => {
  ...
  ionViewWillEnter() {
    this.annonceService
      .loadMesAnnonces(+this.utilisateur.id)
      .then((annonces: Annonce[]) => {
    ...
    this.camera
      ...
      .then((imageData) => {
        if (imageData) {
          ...
          let modal = this.modalCtrl.create(AjoutAnnoncePage, {
            image: imageURL,
          });
          modal.onDidDismiss(() => {
